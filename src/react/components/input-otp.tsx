...
const SLOT_VARIANT_STYLES: Record<InputOTPSlotVariant, string> = {
  ...
  underline:
    "border-0 border-b-2 border-b-white/60 rounded-none bg-transparent pb-1",
};
...
const ACCENT_COLORS: Record<
  InputOTPAccent,
  { ring: string; caret: string; border: string }
> = {
  iris: {
...
>(({ index, className, ...props }, ref) => {
  ...
  const { size, variant, status, accent, disabled, showCursor, cursorMode } =
    useOTPStyle();
  ...
  const getFilledStyles = () => {
    ...
    if (variant === "underline") {
      return cn(
        accentStyles.border.replace("border-", "border-b-"),
        "border-b-opacity-80"
      );
    }
    return "";
...
>(({ className, variant = "dash", ...props }, ref) => (
  <div
    ...
    role="separator"
    className={cn(
      "flex items-center justify-center text-white/50 px-2",
      className
    )}
    {...props}
  >
...
>(({ className, ...props }, ref) => (
  <p
    ref={ref}
    className={cn("text-sm text-white/50 mb-3", className)}
    {...props}
  />
));
...
>(({ className, status = "default", children, ...props }, ref) => {
  ...
  return (
    <p
      ref={ref}
      className={cn(
        "flex items-center gap-2 text-sm mt-3",
        STATUS_MESSAGE_STYLES[status],
        className
      )}
      {...props}
    >
...
const InputOTPField = React.forwardRef<
  React.ElementRef<typeof OTPInput>,
  InputOTPFieldProps
>(
  (
    ...
    return (
      <div className={cn("w-fit", className)}>
        ...
        {description && (
          <InputOTPDescription>{description}</InputOTPDescription>
        )}
        <InputOTP ref={ref} status={status} {...props} />
        {message && (
          <InputOTPMessage status={status}>{message}</InputOTPMessage>
        )}
      </div>
    );
  }
